...
* AOV网
    ...
    * 所有活动可排列成一个线性序列，使得每个活动的所有前驱活动都排在该活动的前面，我们把此序列叫做拓扑序列。只有完成了所有前驱事件后才可以进行后续事件。
    * AOV网的拓扑序列不是唯一的
* AOE网
...
* 交换排序
    ...
    * [快速排序](https://blog.csdn.net/asdfsadfasdfsa/article/details/83009869)：挖坑填数+分而治之
        ...
        * 时间复杂度O(nlog2n)。 空间复杂度O(nlog2n)。 不稳定。 顺序存储，链式存储
* 选择排序
    ...
    * 堆排序
        ...
        * 时间复杂度O(nlog2n)。 空间复杂度O(1)。 不稳定。 顺序存储，链式存储
...
希尔|数学界|证明|不出来|O(1)|不稳定
快速|O(Nlog2N)|O(Nlog2N)|O(n^2)|O(log2N)|不稳定
堆|O(Nlog2N)|O(Nlog2N)|O(Nlog2N)|O(1)|不稳定
二路归并|O(Nlog2N)|O(Nlog2N)|O(Nlog2N)|O(n)|稳定
